...
| **4. Repository Hygiene Before Push** | Before ANY push to GitHub, verify that no personal notes, scratch files, or folders matching `*gitignore*`, `*notes*`, or `ZZ *` patterns are staged or committed. The `.gitignore` must exclude these paths. Never push content from the `ZZ Notes - Git Ignore/` folder or any similarly named notes directory. | "Am I about to push? Have I checked `git status` for notes/scratch files? Is `.gitignore` up to date?" |
...
| **7. Never Commit Without Checking Runtime Errors** | Static analysis verifies the code looks right. Runtime verification confirms it works. Both must pass before commit. Check error tracker, application logs, and health endpoints. | "Did a verification sub-agent check error tracker, application logs, and health endpoints? All clean?" |
