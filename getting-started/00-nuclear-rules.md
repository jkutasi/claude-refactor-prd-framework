...
| **3. Slices Ship Complete** | A slice is not done until all Gherkin pass, all QA pass (including Whiskey Team + UX Sense Check), all peer review is resolved, Red Team pre-build gate passed, Goal Achievement Test passed via agent-browser, coverage ≥ 90% on business logic, docs updated, and ALL review artifact files exist on disk. The user only sees completed, fully-vetted slices. Never present unreviewed work and never defer QA to "after user tests." | "Is Slice N completely done? Every gate passed? Do ALL review artifacts exist? Am I presenting DONE work — not a draft?" |
