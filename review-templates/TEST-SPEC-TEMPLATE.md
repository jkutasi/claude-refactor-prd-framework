...
> **Phase B artifact.** This file is created during Phase B (Gherkin Audit + Test Specification) and documents the test plan for this slice. Test-writer sub-agents (spawned by QA Lead) write the actual test code. Implementation coders are firewalled from this phase.
...
| 2 | {STORY_ELEMENT_2} | {SCENARIO_NAME} | `features/slice-{N}-{feature}.feature` | COVERED / GAP |
| 3 | {STORY_ELEMENT_3} | {SCENARIO_NAME} | `features/slice-{N}-{feature}.feature` | COVERED / GAP |
| ... | ... | ... | ... | ... |
...
**Max 3 cycles.** If gaps remain after 3 cycles, owner sign-off is required to proceed.
...
Before test-writers write tests, the Architect defines skeletal interfaces so tests can import cleanly.
...
| {MODULE_2} | `class {ClassName}` | All methods = `pass` | `{FILE_PATH}` |
| ... | ... | ... | ... |

- [ ] All skeletal interfaces created
- [ ] All interfaces importable without errors
- [ ] All methods raise `NotImplementedError` or return `pass` (no real logic)
...
| 2 | `tests/integration/test_{feature}.py` | {DESCRIPTION} | {COMPONENTS} | RED / PASS (bad) |
| ... | ... | ... | ... | ... |
...
All tests MUST be RED before proceeding to Phase C (Implementation).

**What RED means:**
- Tests that crash on `ImportError` / `ModuleNotFoundError` = valid red (module doesn't exist yet)
- Tests that fail on assertions = valid red (stub returns wrong value)
- Tests that PASS = bad test (testing nothing) — MUST be fixed

### Red Phase Results

| Test Category | Total Tests | RED (valid) | PASS (bad — fix required) | ERROR (harness issue) |
|---|---|---|---|---|
...
- [ ] Zero tests PASS (any passing test = bad test, must fix)
- [ ] Zero harness errors (test framework misconfiguration)
